...
Deno.serve(async (req: Request) => {
  try {
    ...
    const { data: allEmbeddings, error: allEmbeddingsError } = await supabaseClient.from('resume_embeddings').select('id');

    if (allEmbeddingsError) {
      return new Response(JSON.stringify({ error: 'Failed to get all embeddings' }), { 
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    if (resumesToInsert.length === 0) {
      return new Response(JSON.stringify({ success: true, message: 'No new resumes to process' }), { 
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      });
    }
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/insert-embedded-resumes' \
